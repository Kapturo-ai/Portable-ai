...
# Sticker-card repository portability workflow
...
4. Create a deterministic `sticker-card.json` report. Do not include credentials or machine-specific absolute paths.
5. Generate the portable skill under `skills/sticker-card/` and host projections only through the provided build command.
